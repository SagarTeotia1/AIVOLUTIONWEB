...
export const Banner = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center py-20 overflow-hidden" id="home">
      <div className="absolute top-0 left-0 w-[300px] h-[300px] bg-primary/20 rounded-full blur-[100px] -z-10"></div>
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="w-full md:w-7/12 animate__animated animate__fadeIn">
            <span className="inline-block px-4 py-2 mb-6 border border-secondary/50 text-secondary font-mono text-sm uppercase">
              Innovating the Future
            </span>
            <h1 className="text-5xl md:text-7xl font-bold text-white mb-6">
              We are AIvolution <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">
                {text}<span className="border-r-4 border-white ml-1 animate-pulse"></span>
              </span>
            </h1>
            <p className="text-gray-400 text-lg md:text-xl mb-10 max-w-xl border-l-4 border-primary/50 pl-6 py-2">
              The official Artificial Intelligence Society of MAIT, Delhi. A community of curious, creative, and ambitious minds.
            </p>
            <a href="https://chat.whatsapp.com/IAwNmxi4lyG8z52Zh3QTbU" target="_blank" rel="noopener noreferrer" className="inline-flex items-center group gap-3 text-xl font-bold text-white">
              <span className="border-b-2 border-white group-hover:border-primary transition-all">Join the Revolution</span> 
              <ArrowRightCircle size={25} className="group-hover:translate-x-2 transition-all" />
            </a>
          </div>
          <div className="w-full md:w-5/12 animate__animated animate__zoomIn">
            <img src={headerImg} alt="Logo" className="max-w-[350px] md:max-w-full h-auto mx-auto drop-shadow-2xl" />
          </div>
        </div>
      </div>
    </section>
